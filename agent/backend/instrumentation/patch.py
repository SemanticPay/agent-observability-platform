...
2. Create Phare spans that mirror ADK's telemetry
...
from agent.backend.instrumentation.common.common import SpanAttributes, ToolAttributes, MessageAttributes, AgentAttributes
...
def _runner_run_async_wrapper(phare_tracer):
...
def _base_agent_run_async_wrapper(phare_tracer):
    def actual_decorator(wrapped, instance, args, kwargs):
        async def new_function():
            agent_name = instance.name if hasattr(instance, "name") else "unknown"
            span_name = f"adk.agent.{agent_name}"

            with phare_tracer.start_as_current_span(span_name, kind=SpanKind.CLIENT) as span:
                span.set_attribute(SpanAttributes.PHARE_SPAN_KIND, "agent")
                span.set_attribute(SpanAttributes.LLM_SYSTEM, "gcp.vertex.agent")
                span.set_attribute(SpanAttributes.PHARE_ENTITY_NAME, "agent")
...
def _base_llm_flow_call_llm_async_wrapper(phare_tracer):
    def actual_decorator(wrapped, instance, args, kwargs):
        async def new_function():
            ...
            with phare_tracer.start_as_current_span(span_name, kind=SpanKind.CLIENT) as span:
                span.set_attribute(SpanAttributes.PHARE_SPAN_KIND, "request")
                span.set_attribute(SpanAttributes.LLM_SYSTEM, "gcp.vertex.agent")
                span.set_attribute(SpanAttributes.PHARE_ENTITY_NAME, "request")
...
def _adk_trace_tool_call_wrapper(phare_tracer):
    @wrapt.decorator
...
def _adk_trace_tool_response_wrapper(phare_tracer):
    @wrapt.decorator
    def wrapper(wrapped, instance, args, kwargs):
        # Call original to preserve ADK behavior
        result = wrapped(*args, **kwargs)
...
def _adk_trace_call_llm_wrapper(phare_tracer):
    @wrapt.decorator
    def wrapper(wrapped, instance, args, kwargs):
...
def _adk_trace_send_data_wrapper(phare_tracer):
    @wrapt.decorator
    def wrapper(wrapped, instance, args, kwargs):
        # Call original to preserve ADK behavior
        result = wrapped(*args, **kwargs)
...
def _finalize_model_response_event_wrapper(phare_tracer):
    def actual_decorator(wrapped, instance, args, kwargs):
...
def _call_tool_async_wrapper(phare_tracer):
    ...
    def actual_decorator(wrapped, instance, args, kwargs):
        async def new_function():
            ...
            with phare_tracer.start_as_current_span(span_name, kind=SpanKind.CLIENT) as span:
                span.set_attribute(SpanAttributes.PHARE_SPAN_KIND, "tool")
                span.set_attribute(SpanAttributes.LLM_SYSTEM, "gcp.vertex.agent")
                span.set_attribute(SpanAttributes.PHARE_ENTITY_NAME, "tool")
...
def _patch(module_name: str, object_name: str, method_name: str, wrapper_function, phare_tracer):
    """Helper to apply a patch and keep track of it."""
    try:
        module = __import__(module_name, fromlist=[object_name])
        obj = getattr(module, object_name)
        wrapt.wrap_function_wrapper(obj, method_name, wrapper_function(phare_tracer))
...
def _patch_module_function(module_name: str, function_name: str, wrapper_function, phare_tracer):
    """Helper to patch module-level functions."""
    try:
        module = __import__(module_name, fromlist=[function_name])
        wrapt.wrap_function_wrapper(module, function_name, wrapper_function(phare_tracer))
...
def patch_adk(phare_tracer):
    """Apply all patches to Google ADK modules."""
    logger.debug("Applying Google ADK patches for Phare instrumentation")
    ...
    # Patch methods that create top-level Phare spans
    # Skip runner patching - we don't want adk.runner spans
    _patch("google.adk.agents.base_agent", "BaseAgent", "run_async", _base_agent_run_async_wrapper, phare_tracer)

    # Patch ADK's telemetry functions to add attributes to Phare spans
    _patch_module_function("google.adk.telemetry", "trace_tool_call", _adk_trace_tool_call_wrapper, phare_tracer)
    _patch_module_function(
        "google.adk.telemetry", "trace_tool_response", _adk_trace_tool_response_wrapper, phare_tracer
    )
    _patch_module_function("google.adk.telemetry", "trace_call_llm", _adk_trace_call_llm_wrapper, phare_tracer)

    _patch_module_function("google.adk.telemetry", "trace_send_data", _adk_trace_send_data_wrapper, phare_tracer)
    ...
    _patch_module_function(
        "google.adk.flows.llm_flows.functions", "__call_tool_async", _call_tool_async_wrapper, phare_tracer
    )
