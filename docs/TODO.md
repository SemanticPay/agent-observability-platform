...
- [x] Fix PaymentQR props in CopilotKitPage.tsx:
  - Changed `amount` → `amountSats`, `onPaymentConfirmed` → `onConfirmPayment`, added `onCancel`

#### 10.1.2 PaymentStatus Props (CopilotKitPage.tsx)
- [x] Fix PaymentStatus props in CopilotKitPage.tsx:
  - Changed `status="confirmed"` → `status="paid"`, removed `message` prop

#### 10.1.3 LoginForm Props (CopilotKitPage.tsx)
- [x] Fix LoginForm props in CopilotKitPage.tsx:
  - Now uses AuthContext's `login` and `register` functions properly

#### 10.1.4 RenewalForm Missing onCancel
- [x] Add missing `onCancel` prop to RenewalForm in CopilotKitPage.tsx:
  - Added: `onCancel={() => renewalFlow.cancelRenewal()}`

### 10.2 API Response Missing expires_at ✅

#### 10.2.1 CreateTicketResponse Missing expires_at
- [x] Add `expires_at: str` to CreateTicketResponse in types.py

#### 10.2.2 Update tickets.py to Return expires_at
- [x] Update `create_new_ticket` endpoint to include `expires_at` from invoice

### 10.3 Expired Invoice Handling ✅

#### 10.3.1 Update confirm-payment to Handle Expired Invoices
- [x] Update `confirm_ticket_payment` in routes/tickets.py to check `payment_status.expired` and return 'expired' status

#### 10.3.2 Update ConfirmPaymentResponse for Expired Status
- [x] Update ConfirmPaymentResponse type to include 'expired': `status: Literal['pending', 'paid', 'expired']`

#### 10.3.3 Update Frontend to Handle Expired Status
- [x] Update useRenewalFlow.ts confirmPayment to handle 'expired' response with error message

### 10.4 V2-DESIGN.md Accuracy Updates ✅

#### 10.4.1 Document Error Response Format
- [x] V2-DESIGN.md Section 4.4 updated to reflect actual error format
  - Errors return `{"detail": ...}` with simple string or structured object

#### 10.4.2 confirm-payment Response Format
- [x] V2-DESIGN.md already had `status: "expired"` case documented

### 10.5 Minor Fixes (Deferred - Low Priority)

#### 10.5.1 PaymentQR Expiry Countdown
- [ ] Optional: Add expiry countdown display to PaymentQR component (nice-to-have)

#### 10.5.2 PaymentStatus Expired State
- [ ] Optional: Add 'expired' status styling to PaymentStatus component (nice-to-have)
